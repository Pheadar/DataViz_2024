...
if not filtered_data.empty:
    ...
    i = 0
    if f"{driver}_data{i}.csv" in os.listdir("treated_data/"):
        while f"{driver}_data{i}.csv" in os.listdir("treated_data/"):
            i += 1
    filtered_data.to_csv(f"treated_data/{driver}_data{i}.csv", index=False)
    print(f"Data saved to treated_data/{driver}_data{i}.csv")
